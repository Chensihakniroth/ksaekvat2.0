...
async function download(name, fileName) {
  // Using a more direct URL format for Fandom images
  ...
  try {
    const response = await axios({
      ...
      method: 'GET',
      responseType: 'stream',
      maxRedirects: 10, // Ensure it follows redirects
      headers: {
...
async function run() {
  await download('Ultra_Ball', 'ultra_ball_art.png');
  await download('Master_Ball', 'master_ball_art.png');
