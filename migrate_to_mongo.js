...
async function migrate() {
    ...
    if (fs.existsSync(poolPath)) {
        console.log('Checking Gacha Pool migration status...');
        const existingCharactersCount = await Character.countDocuments();

        if (existingCharactersCount > 0) {
            console.log('Gacha Pool already migrated. Skipping. ✅');
        } else {
            ...
            for (const [game, rarities] of Object.entries(poolData)) {
                for (const [rarity, items] of Object.entries(rarities)) {
                    for (const item of items) {
                        // Check if the item already exists by name and game to avoid duplicate inserts
                        const existingItem = await Character.findOne({ name: item.name, game: game });
                        if (!existingItem) {
                            await Character.findOneAndUpdate({ name: item.name }, {
                                name: item.name,
                                game: game,
                                rarity: rarity,
                                emoji: item.emoji || '✨',
            ...
            console.log(`Gacha Pool migration complete (${itemCount} new items added)! ✅`);
        }
    }
