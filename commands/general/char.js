...
module.exports = {
    ...
    async execute(message, args, client) {
        ...
        const createEmbed = () => {
            ...
            const description = pageItems.map(c => {
                const star = c.rarity === 5 ? '🟡' : (c.rarity === 4 ? '🟣' : '🔵');
                const gameEmoji = c.game === 'genshin' ? '🍃' : (c.game === 'hsr' ? '🚂' : (c.game === 'wuwa' ? '🌊' : '🎮'));
        ...
        collector.on('collect', async i => {
            if (i.user.id !== message.author.id) return i.reply({ content: "This isn't your collection, darling! (っ˘ω˘ς)", flags: [MessageFlags.Ephemeral] });
