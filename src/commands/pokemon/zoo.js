...
async function createPCBoxImage(boxName, boxPokemons) {
  ...
  const spritePromises = boxPokemons.slice(0, 30).map(async (pkmn, i) => {
    ...
    // Rarity glow ring
    const glowSvg =
      Buffer.from(`<svg xmlns="http://www.w3.org/2000/svg" width="${cellSize - 6}" height="${cellSize - 6}">
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    try {
      ...
      const boxes = Array.from({ length: Math.ceil(allCaught.length / 30) }, (_, i) =>
        allCaught.slice(i * 30, i * 30 + 30)
      );
      ...
      message.reply({
        embeds: [
          new EmbedBuilder()
            ...
            .setTitle('(｡•́︿•̀｡) Oopsie!')
            .setDescription(
              "Something went wrong while trying to open your PC box, sweetie. Mommy's looking into it! (っ˘ω˘ς)"
            ),
        ],
      });
    }
