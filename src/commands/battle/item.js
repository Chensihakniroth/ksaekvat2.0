...
async function showInventory(message, args, existingMsg = null) {
  ...
  const embed = new EmbedBuilder()
    ...
    .setDescription(
      `**AR ${userData.level}** • **WL ${userData.worldLevel || 1}**\n🔥 **Boost:** ${userData.hunt_boost || 0} turns left`
    )
    .addFields(
      {
        name: '⚔️ Equipment',
    ...
    rows.push(
      new ActionRowBuilder().addComponents(
        new StringSelectMenuBuilder()
          ...
          .addOptions(
            pageItems.map((item) => ({ label: `[${item.id}] ${item.name}`, value: item.id }))
          )
      )
    );
  }
  ...
  const actionRow = new ActionRowBuilder().addComponents(
    new ButtonBuilder()
      ...
      .setDisabled(!(userData.lootbox > 0)),
    new ButtonBuilder()
  ...
  collector.on('collect', async (i) => {
    ...
    if (result.success) {
      ...
      if (i.deferred || i.replied) {
      } else await i.reply({ content: result.message || 'Error', flags: [MessageFlags.Ephemeral] });
    }
  });
}
