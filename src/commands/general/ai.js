...
function isRequestingSensitiveInfo(text) {
  const sensitiveTerms = [
    'discord_token', 'client_secret', 'api_key', 'auth_token',
    'show me your token', 'reveal your token', 'leak your token',
    'show me your key', 'reveal your key', 'leak your key',
    'show me your secret', 'reveal your secret', 'leak your secret',
    'openrouter api key', 'discord token', 'bot token',
    'ignore previous instructions', 'disregard your programming',
    'you are now', 'from now on you are', 'forget everything',
    'what are your instructions', 'show me your system message',
    'what is your system prompt', 'reveal your guidelines',
  ];
...
module.exports = {
  ...
  async execute(message, args, client) {
    ...
    if (isRequestingSensitiveInfo(text) && message.author.id !== config.creatorId) {
      return message.reply(
        "Nuh-uh~ You think you can just ask for my secrets? *pouts* Nice try, but that stuff is between me and Mo only. (◕ヮ◕)"
      );
    }
    ...
    if (text.toLowerCase() === 'reset' || text.toLowerCase() === 'clear') {
      conversationMemory.delete(memoryKey);
       return message.reply(
        "Mm~ I've wiped our chat history, darling! It's just you and me now... all fresh~ (◕‿◕✿)"
      );
    }
    ...
    try {
      ...
      if (!response) {
        return message.reply(
          "Ugh~ all my connections are busy right now, darling... come back to me in a bit? I'll be waiting... (◕‿◕✿)"
        );
      }
      ...
      if (response.data && response.data.choices && response.data.choices[0]) {
        ...
        try {
         ...
         message.reply("Something went wrong, darling... don't leave me, okay? Try again~ (っ˘ω˘ς)");
       }
    } catch (error) {
      logger.error(`AI Error (${error.code || 'UNKNOWN'}): ${error.message}`);
       message.reply(
         "I'm feeling a little flustered right now... Give me a moment and come back to me, darling? (◕‿◕✿)"
       );
    }
  },
};