  ...
  async execute(message, args, client) {
    ...
    if (!charName) {
      return message.reply(
        `❓ Tell Mommy which character you want to inspect! (｡♥‿♥｡)\n**Usage:** \`kcd <name>\`\n*Example: \`kcd Raiden Shogun\`*`
      );
    }
    ...
    const match = inventory.find(
      (i) =>
        (i.type === 'character' || !i.type) && i.name.toLowerCase().includes(charName.toLowerCase())
    );

    if (!match) {
      return message.reply(
        `❌ Sweetie, you don't have **${charName}** in your collection yet! Use \`kwish\` to get them~ (っ˘ω˘ς)`
      );
    }

    // Count duplicates for constellation/eidolon level
    const allCopies = inventory.filter(
      (i) => (i.type === 'character' || !i.type) && i.name === match.name
    );
    ...
    collector.on('collect', async (i) => {
      ...
      if (!freshUser.team) freshUser.team = [];

      if (i.customId === 'cd_add_team') {
        if (freshUser.team.length >= 4) {
          return i.followUp({
            content:
              '🚫 Your team is already full! Remove someone with `kteam remove <slot>`. (っ˘ω˘ς)',
