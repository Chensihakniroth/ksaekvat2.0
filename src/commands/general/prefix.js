        ...
        await interaction.update({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `Your prefix has been reset to the default \`${config.prefix[1]}\`, darling! Everything is back to normal~ ヽ(>∀<☆)ノ`
              ),
          ],
          components: [],
        });
        ...
        await prompt.edit({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `Your prefix must be **${MAX_PREFIX_LEN} characters or fewer**. Please try again with \`${config.prefix[1]}prefix\`! (｡•́︿•̀｡)`
              ),
          ],
          components: [],
        });
        ...
        await prompt.edit({
          embeds: [
            new EmbedBuilder()
              ...
              .setDescription(
                `That prefix contains a forbidden character: ${FORBIDDEN_PREFIXES.map((p) => `\`${p}\``).join(' ')}\nPlease try again! (｡•́︿•̀｡)`
              ),
          ],
          components: [],
        });
