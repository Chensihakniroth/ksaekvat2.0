  ...
  async execute(message, args, client) {
        ...
        embeds: [
          {
            ...
            description:
              "Please specify how much you'd like to bet, sweetie. (｡♥‿♥｡) \n**Usage:** `Krps <amount/all>`",
          },
        ],
      });
    }
    ...
    collector.on('collect', async (i) => {
      ...
      const finalEmbed = new EmbedBuilder()
        .setColor(
          winMultiplier > 1 ? colors.success : winMultiplier === 1 ? colors.warning : colors.error
        )
    ...
    collector.on('end', async (collected, reason) => {
      ...
      await QuestService.updateProgress(message.author.id, 'RPS', 1);
      await QuestService.updateWeeklyProgress(message.author.id, 'RPS', 1);
    });
  },
};
