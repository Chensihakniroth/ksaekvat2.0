...
router.get('/stats', verifyOwner, (req: Request, res: Response) => {
  ...
  res.json({
    ...
    stats: {
      ...
      guilds: client.guilds.cache.map((g: any) => ({
        id: g.id,
        name: g.name,
...
router.post('/execute', verifyOwner, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (normalized.startsWith('guild invite') || normalized.startsWith('invite')) {
      ...
      const guild = client.guilds.cache.get(targetGuildId) || client.guilds.cache.find((g: any) => g.name.toLowerCase().includes(targetGuildId.toLowerCase()));
      ...
      // Generate invite
      const channel = guild.rulesChannel || guild.systemChannel || guild.channels.cache.find(
        (ch: any) => ch.type === 0 && ch.permissionsFor(guild.members.me).has('CreateInstantInvite')
      );
...
router.post('/guild/:guildId/invite', verifyOwner, async (req: Request, res: Response) => {
  ...
  try {
    ...
    if (invites) {
      const existing = invites.find((inv: any) => inv.inviter?.id === client.user.id);
      if (existing) {
