...
function verifyAuth(req: any, res: Response): any {
...
router.get('/list', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const dbConfigs = await GuildConfig.find({ guildId: { $in: mutualGuildIds } }).lean();
    const configMap = new Map(dbConfigs.map((c) => [c.guildId, c]));

    const formattedGuilds = adminGuilds.map((g: any) => {
...
router.get('/:guildId', async (req: Request, res: Response) => {
  ...
  try {
    ...
    const channels = guild.channels.cache
      .filter((c: any) => c.type === 0) // GuildText channels
      .map((c: any) => ({
        id: c.id,
        name: c.name,
      }))
...
router.post('/:guildId/update', async (req: Request, res: Response) => {
  const user = verifyAuth(req, res);
  if (!user) return;
  ...
  try {
    ...
    if (welcomeChannel !== undefined) config.welcomeChannel = welcomeChannel;
    if (welcomeMessage !== undefined) config.welcomeMessage = welcomeMessage;
    if (loggingEnabled !== undefined) config.loggingEnabled = loggingEnabled;
