...
router.get('/:userId', async (req: Request, res: Response) => {
  try {
    ...
    const hydratedFavorites = await Promise.all(
      (user.profileTheme?.favorites || []).map(async (fav: any) => {
...
router.post('/update', async (req: any, res: Response) => {
  ...
  try {
    ...
    user.profileTheme = {
      ...
      bannerPosition:
        bannerPosition !== undefined ? bannerPosition : user.profileTheme.bannerPosition,
      ...
      publicLeaderboard:
        publicLeaderboard !== undefined ? publicLeaderboard : user.profileTheme.publicLeaderboard,
