...
class QuestService {
   ...
   */
  public async generateWeeklyQuests(userId: string) {
    const user = await User.findOne({ id: userId });
    if (!user) return null;
    ...
    user.weeklyQuests = newWeeklyQuests as any;
   ...
   */
  public async updateWeeklyProgress(userId: string, type: string, amount = 1) {
    const user = await User.findOne({ id: userId });
    if (!user) return;
