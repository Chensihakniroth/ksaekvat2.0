...
class CombatService {
  ...
  public generateEnemy(userLevel: number, worldLevel: number): Enemy {
    ...
    switch(enemyClass) {
      case 'TANK': hpMod = 1.6; defMod = 1.5; atkMod = 0.8; eva = 0.02; break;
      case 'STRIKER': hpMod = 0.8; defMod = 0.6; atkMod = 1.8; eva = 0.15; break;
      case 'SUPPORT': hpMod = 1.1; defMod = 0.9; atkMod = 1.0; eva = 0.08; break;
      case 'BOSS': hpMod = 3.2; defMod = 1.3; atkMod = 1.6; eva = 0.10; break;
    }
  ...
  public getEnemyAction(enemy: Enemy, team: any[]): { type: 'ATTACK' | 'SKILL', damage: number, log: string, metadata?: any } {
    ...
    if (enemy.isEnraged) damageMultiplier = 1.3;
   ...
   */
  public calculateAttackDamage(attackerAtk: number, targetDef: number, critRate: number = 0.05, critDmg: number = 0.5, targetEvasion: number = 0): number {
    // Evasion check
    if (Math.random() < targetEvasion) return 0; // Miss!
   ...
   * Assist Attack Logic.
   */
  public calculateFollowUp(attacker: any, team: any[]): { name: string, damage: number } | null {
  ...
  public calculateRewards(enemy: Enemy, betAmount: number, won: boolean): BattleRewards {
    if (!won) return { money: 0, exp: 25 }; 

    let multiplier = 1.0;
    if (enemy.class === 'BOSS') multiplier *= 4.0;
    if (enemy.rarity === 'LEGENDARY') multiplier *= 3.0;
