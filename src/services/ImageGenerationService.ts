...
export class ImageGenerationService {
  ...
  static createEmptySlotRaw(width: number, height: number): Buffer {
    ...
    for (let y = 0; y < height; y++) {
      for (let x = 0; x < width; x++) {
        ...
        const borderDist = 4;
        const onBorderX =
          (x >= borderDist && x <= borderDist + 3) ||
          (x >= width - borderDist - 4 && x <= width - borderDist - 1);
        const onBorderY =
          (y >= borderDist && y <= borderDist + 3) ||
  ...
  static async createTeamImage(userData: any, teamCharacters: any[]): Promise<string | null> {
    ...
    try {
      ...
      const bgBuffer = await sharp(rawBgArray, {
      ...
      for (let i = 0; i < 4; i++) {
        ...
        const item =
          charName && teamCharacters ? teamCharacters.find((c: any) => c.name === charName) : null;

        let slotBuffer: Buffer;
        const rawMaskArray = this.createCardMaskRaw(cardWidth, cardHeight, 24);
        const cardMaskBuffer = await sharp(rawMaskArray, {
        ...
        if (item) {
          try {
            ...
            if (game === 'genshin') {
              ...
              try {
                const jmpRes = await axios.get(imageUrl, {
                ...
                if (
                  jmpRes.headers['content-type']?.includes('image/') &&
                ...
                if (pageId !== '-1' && res.data.query.pages[pageId].imageinfo) {
                  const wikiRes = await axios.get(res.data.query.pages[pageId].imageinfo[0].url, {
                    responseType: 'arraybuffer',
                    timeout: 8000,
                  });
            ...
            const gradientBg = await sharp(rawGradientArray, {
              raw: { width: cardWidth, height: cardHeight, channels: 4 },
            })
              .png()
              .toBuffer();
          ...
          const emptyBaseBuffer = await sharp(emptyRawArray, {
            raw: { width: cardWidth, height: cardHeight, channels: 4 },
          })
            .png()
            .toBuffer();
