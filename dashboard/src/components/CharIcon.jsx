...
const localIcons = import.meta.glob('../assets/char_icon/**/*.webp', { eager: true });

export default function CharIcon({ name, game, rarity, emoji, className = 'char-icon-img' }) {
  const [error, setError] = useState(false);

  // 1. Determine local path if possible
  // Pattern: ../assets/char_icon/[game]/[rarity]/[Name_With_Underscores].webp
  const localUrl = useMemo(() => {
    ...
    const path = `../assets/char_icon/${g}/${r}/${n}.webp`;
  ...
  if (error || !name) {
    return <div className="char-icon-fallback">{emoji || getFallbackEmoji(rarity)}</div>;
  }
