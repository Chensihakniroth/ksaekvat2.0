        ...
        <div style={{ display: 'flex', flexWrap: 'wrap' }}>
          ...
          >
            ...
            >
              <div>
                <div
                  style={{ fontSize: '0.6rem', fontWeight: 900, opacity: 0.3, marginBottom: '8px' }}
                >
                  ROLE
                </div>
              ...
              <div>
                ...
                <div style={{ fontSize: '1.1rem', fontWeight: 800, color: 'var(--cyber-cyan)' }}>
                  LEVEL {char.rarity}
                </div>
              </div>
            </div>
...
export default function ProfilePage() {
  ...
  const isSpotify =
    theme.music && typeof theme.music === 'string' && theme.music.includes('spotify.com');
  const spotifyTrackId = isSpotify
    ? theme.music.split('/').filter(Boolean).pop()?.split('?')[0]
    : null;
        ...
        <div className="portfolio-banner-wrap">
          {theme.banner ? (
            <img
              ...
              style={{ objectPosition: `center ${theme.bannerPosition || '50%'}` }}
              alt="Banner"
            />
          ) : (
            <div
              className="banner-placeholder"
              style={{ background: `linear-gradient(135deg, ${accent}20, #000)` }}
            />
          )}
        </div>
          ...
          <div className="portfolio-actions-fb">
            {isOwner && (
              <Link
                to="/dashboard"
                className="btn-v3"
                style={{ backgroundColor: accent, color: '#000', borderColor: accent }}
              >
                <Edit3 size={18} />
      ...
      <div className="portfolio-content-wrap">
        <aside className="portfolio-zen-sidebar">
          <div className="zen-info-card glass-panel">
            ...
            >
              {theme.bio || 'Operative bio pending...'}
            </p>
            <div className="zen-social-list">
              {theme.socials &&
                Object.entries(theme.socials).map(
                  ([key, val]) =>
                    val && (
                      <a
                        key={key}
                        href={val.startsWith('http') ? val : `https://${key}.com/${val}`}
                        target="_blank"
                        rel="noreferrer"
                        className="zen-social-item"
                      >
        ...
        <main className="portfolio-zen-main">
          <div className="portfolio-tabs-fb">
            <button
              ...
              className={`p-tab-fb ${activeTab === 'portfolio' ? 'active' : ''}`}
            >
              Portfolio
            </button>
            <button
              onClick={() => setActiveTab('favorites')}
              className={`p-tab-fb ${activeTab === 'favorites' ? 'active' : ''}`}
            >
              Favorites
            </button>
          </div>
                        ...
                        {item.type === 'art' ? (
                          ...
                          <div className="zen-card-info">
                            ...
                            <p className="zen-card-desc">
                              {item.description || 'Source protocols established.'}
                            </p>
                            <a
                              href={item.url}
                              target="_blank"
                        ...
                        {item.type === 'art' && (
                          <div className="zen-card-info">
                            ...
                            <a
                              ...
                              rel="noreferrer"
                              className="btn-v3 btn-v3-ghost"
                              style={{ marginTop: '15px', width: '100%', fontSize: '0.75rem' }}
                            >
                              OPEN_ARCHIVE
                            </a>
                          </div>
                  ...
                  {favorites.length > 0 ? (
                    favorites.map((fav, i) => (
                      <motion.div
                        ...
                        onClick={() => fav.type === 'character' && setSelectedChar(fav)}
                        style={{ cursor: fav.type === 'character' ? 'pointer' : 'default' }}
                      >
                        <div className="char-visual-fb" style={{ width: '80px', height: '80px' }}>
                          {fav.type === 'character' ? (
                            <CharIcon
                              name={fav.name}
                              game={fav.game?.toLowerCase()}
                              rarity={fav.rarity}
                            />
                          ) : (
                        ...
                        <div className="char-name-fb" style={{ fontSize: '0.8rem' }}>
                          {fav.name}
                        </div>
      ...
      {theme.music && !isSpotify && (
        <div className="music-control-fb">
          <button onClick={toggleMusic} className="music-btn-fb">
            ...
            <span style={{ color: isPlaying ? 'var(--cyber-cyan)' : 'var(--text-dim)' }}>
