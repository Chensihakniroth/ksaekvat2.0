...
export default function ZooPage() {
  ...
  return (
    <div className="zoo-container">
      <div className="bg-ambience">{/* Static background is handled globally in App.jsx */}</div>

      <div className="wrap relative z-10">
        <motion.div
          initial={{ opacity: 0, y: -20 }}
          animate={{ opacity: 1, y: 0 }}
          className="zoo-hero-v4"
        >
          <div className="zoo-title-wrap">
            ...
            <h1 className="zoo-title-v4">
              ZOO <span className="text-cyan-400">REGISTRY</span>
            </h1>
          </div>
          ...
          <div className="zoo-hud-stats">
            ...
            <div className="hud-box">
              ...
              <span className="hud-val legendary">
                {registry.filter((a) => a.rarity === 'legendary').length}
              </span>
            </div>
            <div className="hud-box">
              <span className="hud-label">Mythical</span>
              <span className="hud-val priceless">
                {registry.filter((a) => a.rarity === 'priceless').length}
              </span>
            </div>
          </div>
        </motion.div>
        ...
        {loading ? (
          <div className="zoo-grid-v3">
            {[...Array(12)].map((_, i) => (
              <div key={i} className="skeleton-square glass-panel" />
            ))}
          </div>
        ) : (
          <div className="zoo-grid-v3">
      ...
      <AnimatePresence>
        {selectedAnimal && (
          <BioScanModal animal={selectedAnimal} onClose={() => setSelectedAnimal(null)} />
        )}
      </AnimatePresence>
    </div>
  );
        ...
        <div className="bioscan-details">
          ...
          <div className="bs-databox">
            ...
            <p className="bs-desc">
              ...
              <span className="text-cyan-400"> KSAEKVAT</span> wild zones. High concentration of
              ethereal energy detected within its core matrix."
            </p>
          </div>

          <button
            className="btn-v3 w-full"
            style={{
              background: color,
              border: 'none',
              color: '#fff',
