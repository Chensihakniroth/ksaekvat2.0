...
class PokemonBattleService {
     ...
     */
    async getBaseStats(speciesKey) {
        if (baseStatCache.has(speciesKey))
            return baseStatCache.get(speciesKey);
        ...
        try {
            ...
            const data = {
                hp: stats.find((s) => s.stat.name === 'hp')?.base_stat || 50,
                atk: stats.find((s) => s.stat.name === 'attack')?.base_stat || 50,
                def: stats.find((s) => s.stat.name === 'defense')?.base_stat || 50,
                spAtk: stats.find((s) => s.stat.name === 'special-attack')?.base_stat || 50,
                spDef: stats.find((s) => s.stat.name === 'special-defense')?.base_stat || 50,
                speed: stats.find((s) => s.stat.name === 'speed')?.base_stat || 50,
     ...
     */
    async buildBattlePokemon(userPokemon, side) {
    ...
    simulateBattle(teamA, teamB, maxTurns = 25) {
        ...
        while (turn < maxTurns) {
            ...
            for (const attacker of alive) {
                ...
                // Calculate damage
                const { damage, isCrit } = this.calculateDamage(attacker.level, attacker.atk, target.def, typeMultiplier);
    ...
    async generateWildTeam(avgLevel) {
        ...
        while (team.length < 3) {
            ...
            const wildPokemon = await this.buildBattlePokemon({ speciesKey: species, level: wildLevel, _id: `wild_${species}` }, 'B');
            if (wildPokemon)
                team.push(wildPokemon);
        }
        return team;
    }
     ...
     */
    calculateBattleRewards(won, playerTeam, enemyTeam, faintedXpPenalty = 0.2) {
