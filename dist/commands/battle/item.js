...
async function unequipItem(userId, slot) {
    ...
    const inventory = userData.inventory || [];
    if (!equipped[slot])
        return { success: false, message: `No item equipped in ${slot}` };
...
async function showInventory(message, args, existingMsg = null) {
    ...
    const inventoryList = pageItems.length > 0
        ? pageItems
            .map((item, i) => `**${item.id}** | ${item.name} (${item.rarity})\n└ *${item.perk || 'No perk'}*`)
            .join('\n')
        : '*Your bag is empty.*';
    ...
    const actionRow = new ActionRowBuilder().addComponents(new ButtonBuilder()
        ...
        .setDisabled(!(userData.lootbox > 0)), new ButtonBuilder()
        .setCustomId('un_weapon')
        .setLabel('Unequip Wpn')
        .setStyle(ButtonStyle.Secondary)
        .setDisabled(!equipped.weapon));
    rows.push(actionRow);
    let msg;
    if (existingMsg)
        msg = await existingMsg.edit({ embeds: [embed], components: rows });
    else
        msg = await message.reply({ embeds: [embed], components: rows });
    ...
    collector.on('collect', async (i) => {
        ...
        if (i.customId === 'equip_select') {
            ...
            if (userData.lootbox > 0) {
                ...
                if (rand < 0.6) {
                    ...
                    result = {
                        success: true,
                        message: 'You found a **Hunter Lure**! Next 3 hunts are boosted!',
                    };
                }
                else {
                    // Give Random Weapon
        ...
        if (result.success) {
            ...
            if (i.deferred || i.replied) {
                ...
                await i.reply({ content: result.message || 'Error', flags: [MessageFlags.Ephemeral] });
        }
    });
}
module.exports = {
