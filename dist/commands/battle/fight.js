    ...
    async execute(message, args, client) {
        ...
        if (args[0]?.toLowerCase() === 'upgrade') {
            ...
            if (!(await database.hasBalance(message.author.id, cost))) {
                return message.reply(`System error: Insufficient funds... (・_・ヾ To reach World Level ${nextWorld}, you need **${EconomyService.format(cost)}** <:coin:1480551418464305163>. (ಥ﹏ಥ)`);
            }
            ...
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`Nice! You've reached **World Level ${userData.worldLevel}**! Power ceiling increased. (¬‿¬)`),
                ],
            });
        }
        // --- PREPARE TEAM ---
        ...
        const team = userData.team
            ...
            .filter(Boolean);
        if (team.length === 0)
            return message.reply("Whoa, hold on! (・_・ヾ You don't have a team yet. Build your squad before heading out! (≧◡≦)");
