    ...
    async execute(message, args, client) {
        ...
        if (args[0]?.toLowerCase() === 'upgrade') {
            ...
            if (!(await database.hasBalance(message.author.id, cost))) {
                return message.reply(`Oh darling, you don't have enough coins... (｡•́︿•̀｡) To reach World Level ${nextWorld}, you need **${EconomyService.format(cost)}** <:coin:1480551418464305163>. (っ˘ω˘ς)`);
            }
            ...
            return message.reply({
                embeds: [
                    new EmbedBuilder()
                        ...
                        .setDescription(`Sweetie, you've reached **World Level ${userData.worldLevel}**! You're growing so fast!`),
                ],
            });
        }
        // --- PREPARE TEAM ---
        ...
        const team = userData.team
            ...
            .filter(Boolean);
        if (team.length === 0)
            return message.reply("Sweetie, you don't have a team yet! (｡•́︿•̀｡) Use `kteam add` to find some friends to help you! (◕‿◕✿)");
        // --- HANDLE BET ---
        ...
        const rows = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('attack').setLabel('Attack').setStyle(ButtonStyle.Primary), new ButtonBuilder()
            .setCustomId('combo')
            .setLabel('Combo')
        ...
        collector.on('collect', async (i) => {
            ...
            if (enemy.hp > 0) {
                ...
                if (action.damage > 0) {
                    if (action.metadata?.aoe) {
                        team.forEach(member => {
                            if (member.hp > 0)
                                member.hp -= Math.floor(action.damage * 0.6); // Reduced AOE damage
                        });
                    }
                    else {
                        // Find target member
            ...
            if (enemy.hp <= 0)
                ...
                await battleMsg.edit({ embeds: [getEmbed()], components: [rows] });
        });
