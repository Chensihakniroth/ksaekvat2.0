...
async function createPCBoxImage(boxName, boxPokemons) {
    if (!boxPokemons || boxPokemons.length === 0)
        return null;
    const cols = 6;
    const rows = 5;
    ...
    async execute(message, args, client) {
        ...
        try {
            ...
            const generatePCMessage = async (boxIndex) => {
                ...
                const row = new ActionRowBuilder().addComponents(new ButtonBuilder().setCustomId('zoo_prev').setLabel('◀  Prev').setStyle(ButtonStyle.Secondary).setDisabled(boxIndex === 0), new ButtonBuilder().setCustomId('zoo_next').setLabel('Next  ▶').setStyle(ButtonStyle.Secondary).setDisabled(boxIndex === boxes.length - 1));
