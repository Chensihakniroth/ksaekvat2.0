...
const ImageGenerationService_1 = require("../src/services/ImageGenerationService");
describe('ImageGenerationService - Native Generators', () => {
    describe('createGridBgRaw', () => {
        it('should generate a valid raw RGBA buffer of expected size', () => {
            const width = 100;
            const height = 100;
            const buffer = ImageGenerationService_1.ImageGenerationService.createGridBgRaw(width, height);
    ...
    describe('createCardMaskRaw', () => {
        it('should generate a valid raw RGBA mask buffer', () => {
            ...
            const height = 420;
            const radius = 24;
            const buffer = ImageGenerationService_1.ImageGenerationService.createCardMaskRaw(width, height, radius);
            expect(buffer).toBeDefined();
            expect(buffer.length).toBe(width * height * 4);
    ...
    describe('createEmptySlotRaw', () => {
        it('should generate a valid raw empty slot buffer', () => {
            const width = 280;
            const height = 420;
            const buffer = ImageGenerationService_1.ImageGenerationService.createEmptySlotRaw(width, height);
            expect(buffer).toBeDefined();
            expect(buffer.length).toBe(width * height * 4);
        });
    });
    describe('isValidImageBuffer', () => {
        it('should detect valid PNG buffers', () => {
            ...
            expect(ImageGenerationService_1.ImageGenerationService.isValidImageBuffer(mockPng)).toBe(true);
        });
        it('should detect invalid buffers', () => {
            const mockHtml = Buffer.from("<html><body>Error</body></html>");
            expect(ImageGenerationService_1.ImageGenerationService.isValidImageBuffer(mockHtml)).toBe(false);
        });
    });
});
