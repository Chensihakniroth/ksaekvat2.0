...
const consoleFormat = winston.format.combine(winston.format.timestamp({ format: 'HH:mm:ss' }), winston.format.colorize(), winston.format.printf(({ level, message, timestamp, stack }) => {
...
const logger = {
    ...
    blank: () => console.log(''),
    loader: (message) => {
        process.stdout.write(`${colors.dim}${new Date().toLocaleTimeString()}${colors.reset} ${colors.yellow}⏳${colors.reset} ${message}... `);
        return {
