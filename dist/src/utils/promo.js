...
async function redeemCode(userId, codeInput) {
    ...
    if (!promo)
        return { success: false, message: 'Invalid code! (っ˘ω˘ς)' };
    if (promo.usedBy.includes(userId))
        return { success: false, message: 'You already used this code! (◕‿◕✿)' };
    if (promo.usedBy.length >= promo.maxUses)
        return { success: false, message: 'This code has reached its maximum uses! (っ˘ω˘ς)' };
