...
class ImageGenerationService {
    ...
    static createGridBgRaw(width, height) {
    ...
    static createCardMaskRaw(width, height, radius) {
        const buf = Buffer.alloc(width * height * 4);
        for (let y = 0; y < height; y++) {
            for (let x = 0; x < width; x++) {
                let alpha = 255;
    ...
    static createCardGradientRaw(width, height, rColors) {
        const buf = Buffer.alloc(width * height * 4);
    ...
    static createEmptySlotRaw(width, height) {
        const buf = Buffer.alloc(width * height * 4);
    ...
    static async createTeamImage(userData, teamCharacters) {
        const cardWidth = 280;
        ...
        try {
            ...
            try {
                ...
                const textRes = await axios_1.default.get(textUrl, { responseType: 'arraybuffer' });
